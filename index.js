...
let config;
try {
    config = JSON.parse((await fs.readFile("./config/config.json")).toString());
}
catch (err) {
...
let tasks;
try {
    tasks = JSON.parse((await fs.readFile("./config/tasks.json")).toString()).tasks;
}
catch (err) {
...
try {
    const sessionJson = JSON.parse((await fs.readFile("./config/session.json")).toString());
...
(async () => {
    ...
    let scheduledJobs = []; // Reference to scheduled Jobs, in order to cancel them all if there are changes
...
function restartSchedule(client, jobs, tasks) {
    clearScheduledJobs(jobs);
    const scheduledJobs = startSchedule(client, tasks);
    return scheduledJobs;
}
function startSchedule(client, tasks) {
    let scheduledJobs = [];
...
function clearScheduledJobs(scheduledJobs) {
...
async function checkMuted(client, chatId) {
...
async function muteChat(client, chatId, durationSeconds) {
    const muteUntil = parseFloat(((Date.now() + durationSeconds * 1000) / 1000).toFixed(0));
